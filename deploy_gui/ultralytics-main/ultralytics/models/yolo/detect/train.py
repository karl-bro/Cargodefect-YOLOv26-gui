...
from ultralytics.nn.tasks import CargoDefectDetectionModel, DetectionModel, yaml_model_load
...
class DetectionTrainer(BaseTrainer):
    ...
    def set_class_weights(self):
        ...
        Supports two modes:
        1. Explicit class_weights from args: e.g. class_weights=[1.0, 2.0]
        2. Auto-computed from dataset statistics via cls_pw
        """
        model = self.model
        if hasattr(unwrap_model(model), "student_model"):
            model = unwrap_model(model).student_model

        # Mode 1: explicit class_weights from config
        cw = getattr(self.args, "class_weights", None)
        if cw is not None and isinstance(cw, (list, tuple)) and len(cw) > 0:
            weights = torch.tensor(cw, dtype=torch.float32, device=self.device)
            model.class_weights = weights
            LOGGER.info(f"Class weights (explicit): {weights.cpu().numpy().round(3)}")
            return

        # Mode 2: auto-compute from dataset
        ...
        LOGGER.info(f"Class weights (auto): {model.class_weights.cpu().numpy().round(3)}")
    ...
    def get_model(self, cfg: str | None = None, weights: str | None = None, verbose: bool = True):
        ...
        """
        cfg_str = str(cfg or self.args.model).lower()
        cfg_dict = yaml_model_load(cfg) if cfg and not isinstance(cfg, dict) else (cfg or {})
        head = cfg_dict.get("head", []) if isinstance(cfg_dict, dict) else []
        use_cargo_head = bool(head) and head[-1][2] in ("CargoDefectHead", "CargoInspectionHead")
        model_cls = CargoDefectDetectionModel if use_cargo_head else DetectionModel
        model = model_cls(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
            model.load(weights)
        return model

    def _model_yaml_dict(self):
        """Load model YAML config, including when resuming from a .pt checkpoint."""
        model_arg = getattr(self.args, "model", "")
        if not model_arg:
            return {}
        if str(model_arg).endswith(".pt"):
            from ultralytics.utils.torch_utils import torch_load

            ckpt = torch_load(model_arg)
            train_args = ckpt.get("train_args")
            yaml_path = train_args.get("model") if isinstance(train_args, dict) else getattr(train_args, "model", None)
            if yaml_path and str(yaml_path).endswith((".yaml", ".yml")):
                return yaml_model_load(yaml_path)
            # Fallback: yaml embedded in checkpoint model (train_args.model may point to another .pt)
            model = ckpt.get("model")
            yaml_file = None
            if model is not None:
                yaml_file = getattr(model, "yaml", {}).get("yaml_file") if hasattr(model, "yaml") else None
            if yaml_file and str(yaml_file).endswith((".yaml", ".yml")):
                return yaml_model_load(yaml_file)
            return {}
        return yaml_model_load(model_arg)

    def get_validator(self):
        """Return a DetectionValidator for YOLO model validation."""
        cfg_dict = self._model_yaml_dict()
        head = cfg_dict.get("head", []) if isinstance(cfg_dict, dict) else []
        if head and head[-1][2] in ("CargoDefectHead", "CargoInspectionHead"):
            self.loss_names = "box_loss", "cls_loss", "dfl_loss", "defect_loss", "quality_loss"
        else:
