...
def build_cargodefect_dataset(
    ...
    """Build CargoDefectDataset for multi-task cargo/defect/quality training."""
    data = dict(data)
    data["_mode"] = mode
    yaml_dir = Path(data.get("yaml_file", ".")).parent

    def _resolve_extra_path(value: str) -> str:
        p = Path(value)
        if p.is_absolute():
            return str(p)
    ...
    for key in ("path", "packaging_path", "mvtec_path"):
        if data.get(key):
            data[key] = _resolve_extra_path(str(data[key]))
...
def build_yolo_dataset(
    ...
    if data.get("dataset_type") == "CargoDefectDataset":
        return build_cargodefect_dataset(cfg, img_path, batch, data, mode, rect, stride, fraction)
    pad = 0.0 if mode == "train" else 0.5
