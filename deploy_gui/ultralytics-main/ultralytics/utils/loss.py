...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        # Focal loss for cls branch (fl_gamma > 0 enables it)
        self.fl_gamma = float(getattr(h, "fl_gamma", 0.0))
        if self.fl_gamma > 0:
            self.bce = FocalLoss(gamma=self.fl_gamma, alpha=0.25)
        ...
        # Per-level loss weights (e.g. [1.5, 1.0, 1.0, 0.75] for P2/P3/P4/P5)
        level_w = getattr(h, "level_loss_weights", None)
        if level_w is not None and len(level_w) > 0:
            self.level_loss_weights = torch.tensor(level_w, dtype=torch.float32, device=device)
        else:
            self.level_loss_weights = None
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        # Cls loss with optional class weighting and per-level weighting
        bce_loss = self.bce(pred_scores, target_scores.to(dtype))  # (bs, num_anchors, nc)
        if self.class_weights is not None:
            bce_loss *= self.class_weights

        # Apply per-level loss weights (split bce_loss by level, weight, recombine)
        if self.level_loss_weights is not None and len(self.level_loss_weights) == len(preds["feats"]):
            level_anchors = [f.shape[2] * f.shape[3] for f in preds["feats"]]  # h*w per level
            level_bce_list = torch.split(bce_loss, level_anchors, dim=1)  # split along anchor dim
            weighted_levels = [w * lb for w, lb in zip(self.level_loss_weights, level_bce_list)]
            loss[1] = sum(wl.sum() for wl in weighted_levels) / target_scores_sum
        else:
...
class _SubModelProxy:
    """Proxy model exposing a specific Detect sub-head as model.model[-1] for loss helpers."""

    def __init__(self, parent: torch.nn.Module, head_module: torch.nn.Module):
        from ultralytics.utils import DEFAULT_CFG_DICT, IterableSimpleNamespace

        self.model = _ModelIndexProxy(parent.model, head_module)
        self.args = getattr(parent, "args", None) or IterableSimpleNamespace(**DEFAULT_CFG_DICT)
        self.yaml = parent.yaml
        self.class_weights = getattr(parent, "class_weights", None)

    def parameters(self):
        return self.model.parameters()


class _ModelIndexProxy:
    """Minimal sequence proxy returning a substituted last layer."""

    def __init__(self, sequential: torch.nn.Sequential, last_module: torch.nn.Module):
        self._seq = sequential
        self._last = last_module

    def __getitem__(self, idx: int):
        if idx == -1 or idx == len(self._seq) - 1:
            return self._last
...
class CargoDefectLoss:
    """Combined loss: detection + optional defect detection + quality classification."""

    def __init__(self, model: torch.nn.Module):
        from ultralytics.nn.modules.head import CargoDefectHead
        ...
        self.lambda_quality = model.yaml.get("lambda_quality", 0.5)
        self.lambda_defect = model.yaml.get("lambda_defect", 1.0)
        self.quality_ce = nn.CrossEntropyLoss()
        self.severe_classes = model.yaml.get("quality_thresholds", {}).get("severe_classes", [2, 3, 4])
        self.nc_cargo = int(model.yaml.get("nc_cargo", model.yaml.get("nc", 10) - model.yaml.get("nc_defect", 6)))
        self.nc_quality = int(model.yaml.get("nc_quality", 3))
        self.use_quality_head = model.yaml.get("use_quality_head", True) and isinstance(head, CargoDefectHead)

        detect_module = head.detect if isinstance(head, CargoDefectHead) else head
        detect_proxy = _SubModelProxy(model, detect_module)
        self.detect_loss = E2ELoss(detect_proxy) if self.end2end else v8DetectionLoss(detect_proxy)

        self.defect_loss = None
        if isinstance(head, CargoDefectHead) and head.small_defect is not None and model.yaml.get(
            "use_small_defect_head", True
        ):
            defect_proxy = _SubModelProxy(model, head.small_defect)
            self.defect_loss = E2ELoss(defect_proxy) if self.end2end else v8DetectionLoss(defect_proxy)

        self.quality_head = head.quality if self.use_quality_head and isinstance(head, CargoDefectHead) else None

    def _resolve_outputs(self, preds):
        """Resolve training-style head outputs from train/val forward predictions."""
        if isinstance(preds, dict) and "detect" in preds:
            return preds
        if isinstance(preds, tuple) and len(preds) >= 2 and isinstance(preds[1], dict) and "detect" in preds[1]:
            return preds[1]
        return preds

    def _unwrap_detect_preds(self, detect_preds):
        """Extract loss-ready detect predictions from Detect head output."""
        if isinstance(detect_preds, tuple) and len(detect_preds) == 2:
            return detect_preds[1]
        return detect_preds

    def _run_detect_loss(self, preds, batch):
        outputs = self._resolve_outputs(preds)
        detect_preds = outputs["detect"] if isinstance(outputs, dict) and "detect" in outputs else outputs
        detect_preds = self._unwrap_detect_preds(detect_preds)
        return self.detect_loss(detect_preds, batch)

    def _run_defect_loss(self, preds, batch):
        from ultralytics.data.cargodefect import build_defect_batch

        outputs = self._resolve_outputs(preds)
        defect_preds = outputs["defect"]
        defect_preds = self._unwrap_detect_preds(defect_preds)
        defect_batch = build_defect_batch(batch, self.nc_cargo)
        return self.defect_loss(defect_preds, defect_batch)

    def _run_quality_loss(self, preds, batch):
        from ultralytics.data.cargodefect import quality_targets_from_batch

        outputs = self._resolve_outputs(preds)
        logits = outputs["quality"]
        if isinstance(logits, dict):
            logits = logits["logits"]
        batch["nc_quality"] = self.nc_quality
        targets = quality_targets_from_batch(batch, self.nc_quality)
        loss = self.quality_ce(logits, targets.to(logits.device))
        return loss, loss.detach()

    def __call__(self, preds, batch):
        outputs = self._resolve_outputs(preds)
        loss, loss_items = self._run_detect_loss(preds, batch)

        if isinstance(outputs, dict) and "defect" in outputs and self.defect_loss is not None:
            defect_loss, defect_items = self._run_defect_loss(preds, batch)
            loss = loss + self.lambda_defect * defect_loss
            if isinstance(defect_items, torch.Tensor) and defect_items.numel() > 1:
                loss_items = loss_items + self.lambda_defect * defect_items
            else:
                d = defect_loss.detach().reshape(1) if defect_loss.dim() == 0 else defect_loss.detach().flatten()[:1]
                loss_items = torch.cat([loss_items.flatten(), d])

        if isinstance(outputs, dict) and "quality" in outputs and self.quality_head is not None and self.use_quality_head:
            if "quality_label" in batch or "quality" in batch:
                q_loss, q_detach = self._run_quality_loss(preds, batch)
                loss = loss + self.lambda_quality * q_loss
                q = (self.lambda_quality * q_detach).reshape(1)
                loss_items = torch.cat([loss_items.flatten(), q])

        return loss, loss_items

    def update(self):
        """Update E2E loss schedule if applicable."""
        if self.end2end and hasattr(self.detect_loss, "update"):
            self.detect_loss.update()
            if self.defect_loss is not None and hasattr(self.defect_loss, "update"):
                self.defect_loss.update()


class CargoInspectionLoss:
    """Split-task loss: cargo detection + ROI defect CE + quality BCE."""

    def __init__(self, model: torch.nn.Module):
        from ultralytics.nn.modules.inspection_head import CargoInspectionHead
        ...
        self.head = head
        self.end2end = getattr(model, "end2end", False)
        self.lambda_quality = float(model.yaml.get("lambda_quality", 0.3))
        self.lambda_defect = float(model.yaml.get("lambda_defect", 0.5))
        self.label_smoothing = float(model.yaml.get("label_smoothing", 0.1))
        self.nc_cargo = int(model.yaml.get("nc_cargo", model.yaml.get("nc", 4)))
        self.nc_defect = int(model.yaml.get("nc_defect", 5))
        self.nc_quality = int(model.yaml.get("nc_quality", 2))
        self.use_quality_head = model.yaml.get("use_quality_head", True) and isinstance(head, CargoInspectionHead)

        detect_proxy = _SubModelProxy(model, head.detect)
        self.detect_loss = E2ELoss(detect_proxy) if self.end2end else v8DetectionLoss(detect_proxy)
        self.defect_ce = nn.CrossEntropyLoss(label_smoothing=self.label_smoothing)
        self.quality_focal_gamma = float(model.yaml.get("quality_focal_gamma", 0.0))
        self.quality_focal_alpha = float(model.yaml.get("quality_focal_alpha", 0.25))
        if self.quality_focal_gamma > 0:
            # Focal Loss mode: ignore pos_weight, use alpha+gamma
            self.quality_bce = None
            self.use_focal_loss = True
        else:
            pos_weight = float(model.yaml.get("quality_pos_weight", 2.5))
            self.quality_bce = nn.BCEWithLogitsLoss(
    ...
    def _run_detect_loss(self, preds, batch):
        from ultralytics.data.cargodefect import build_cargo_batch

        outputs = self._resolve_outputs(preds)
        detect_preds = outputs["detect"] if isinstance(outputs, dict) and "detect" in outputs else outputs
        detect_preds = self._unwrap_detect_preds(detect_preds)
        cargo_batch = build_cargo_batch(batch, self.nc_cargo)
        return self.detect_loss(detect_preds, cargo_batch)

    def _run_defect_loss(self, batch):
        logits = self.head.defect_logits(batch, self.nc_cargo)
        if logits.numel() == 0:
            z = batch["img"].new_zeros(())
            return z, z.detach()
        targets = batch["defect_label"].view(-1).long().to(logits.device)
        loss = self.defect_ce(logits, targets)
        return loss, loss.detach()
    ...
    def _run_quality_loss(self, preds, batch):
        ...
        if isinstance(logits, dict):
            logits = logits["logits"]
        targets = quality_targets_from_batch(batch, self.nc_quality).float().to(logits.device)
        if self.nc_quality == 2:
            logit = logits.view(-1)
            if logit.shape[0] != targets.shape[0]:
                logit = logit[: targets.shape[0]]
            if self.use_focal_loss:
                loss = self._focal_bce_loss(logit, targets)
            else:
                loss = self.quality_bce(logit, targets)
        else:
            loss = nn.functional.cross_entropy(logits, targets.long(), label_smoothing=self.label_smoothing)
        return loss, loss.detach()

    def _focal_bce_loss(self, logits, targets):
        """Focal loss for binary classification with logits."""
        p = torch.sigmoid(logits)
        ce = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")
        p_t = p * targets + (1 - p) * (1 - targets)
        alpha_t = self.quality_focal_alpha * targets + (1 - self.quality_focal_alpha) * (1 - targets)
        focal_weight = alpha_t * (1 - p_t) ** self.quality_focal_gamma
    ...
    def __call__(self, preds, batch):
        ...
        if isinstance(self.head, CargoInspectionHead) and self.head.defect_classifier is not None:
            defect_loss, defect_detach = self._run_defect_loss(batch)
            loss = loss + self.lambda_defect * defect_loss
            d = (self.lambda_defect * defect_detach).reshape(1)
            loss_items = torch.cat([loss_items.flatten(), d])

        if isinstance(outputs, dict) and "quality" in outputs and self.use_quality_head:
            if "quality_label" in batch or "quality" in batch:
                q_loss, q_detach = self._run_quality_loss(preds, batch)
                loss = loss + self.lambda_quality * q_loss
                q = (self.lambda_quality * q_detach).reshape(1)
