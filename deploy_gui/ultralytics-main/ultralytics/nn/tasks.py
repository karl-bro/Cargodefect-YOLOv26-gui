...
from ultralytics.nn.modules.edge_branch import EdgeEnhance
from ultralytics.nn.modules.frequency_branch import FrequencyEnhance
from ultralytics.nn.modules.defect_enhance import DefectEnhanceBlock
from ultralytics.nn.modules.head import CargoDefectHead, SmallDefectHead
from ultralytics.nn.modules.inspection_head import CargoInspectionHead
from ultralytics.nn.modules.pgme import PGME
        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                if isinstance(m.f, int):
                    x = y[m.f]
                elif m.f and isinstance(m.f[0], list):
                    x = [[x if j == -1 else y[j] for j in sub] for sub in m.f]
                else:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect() or composite inspection head
        detect_m = m.detect if isinstance(m, (CargoDefectHead, CargoInspectionHead)) else m
        if isinstance(m, (Detect, CargoDefectHead, CargoInspectionHead)):
            s = 256  # 2x min stride
            detect_m.inplace = self.inplace
            ...
            detect_m.training = True  # Setting it to True to properly return strides
            detect_m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # forward
            self.stride = detect_m.stride
            if isinstance(m, CargoDefectHead) and m.small_defect is not None:
                m.small_defect.stride = detect_m.stride
            self.model.train()  # Set model back to training(default) mode
            detect_m.bias_init()  # only run once
        else:
    ...
    def init_criterion(self):
        ...
        if head_name == "CargoInspectionHead":
            from ultralytics.utils.loss import CargoInspectionLoss

            return CargoInspectionLoss(self)
        if head_name == "CargoDefectHead":
            from ultralytics.utils.loss import CargoDefectLoss

            return CargoDefectLoss(self)
        return E2ELoss(self) if getattr(self, "end2end", False) else v8DetectionLoss(self)


class CargoDefectDetectionModel(DetectionModel):
    """Alias for DetectionModel; split-task heads select loss in DetectionModel.init_criterion."""
...
def parse_model(d, ch, verbose=True):
    ...
    nc_defect = d.get("nc_defect", 6)
    nc_quality = d.get("nc_quality", 3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, SmallDefectHead, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is CargoInspectionHead:
            cargo_ch = [ch[x] for x in f]
            nc_cargo = args[0] if isinstance(args[0], int) else nc
            nc_def = args[1] if len(args) > 1 and isinstance(args[1], int) else nc_defect
            nc_qual = args[2] if len(args) > 2 and isinstance(args[2], int) else nc_quality
            use_defect = d.get("use_defect_classifier", True)
            use_quality = d.get("use_quality_head", True)
            quality_thresholds = d.get("quality_thresholds", None)
            roi_size = int(d.get("roi_size", 128))
            args = [
                nc_cargo,
                nc_def,
                nc_qual,
            ...
            c2 = nc_cargo
        elif m is CargoDefectHead:
            cargo_ch = [ch[x] for x in f[0]]
            defect_ch = [ch[x] for x in f[1]] if len(f) > 1 else cargo_ch[:2]
            nc_cargo = args[0] if isinstance(args[0], int) else nc
            nc_def = args[1] if len(args) > 1 and isinstance(args[1], int) else nc_defect
            nc_qual = args[2] if len(args) > 2 and isinstance(args[2], int) else nc_quality
            use_small = d.get("use_small_defect_head", True)
            use_quality = d.get("use_quality_head", True)
            quality_thresholds = d.get("quality_thresholds", None)
            args = [
                nc_cargo,
            ...
            c2 = nc_cargo
        elif m in {PGME, EdgeEnhance, FrequencyEnhance}:
            c2 = ch[f[1]]  # match backbone feature channels (already width-scaled)
            if m is PGME:
                use_edge = d.get("use_edge_branch", args[0] if args else True)
                use_freq = d.get("use_frequency_branch", args[1] if len(args) > 1 else True)
                args = [c2, use_edge, use_freq]
            elif m is EdgeEnhance:
                enabled = d.get("use_edge_branch", args[0] if args else True)
                args = [c2, enabled]
            else:
                enabled = d.get("use_frequency_branch", args[0] if args else True)
                args = [c2, enabled]
        elif m is DefectEnhanceBlock:
            idx = f if isinstance(f, int) else f[0]
            c2 = ch[idx]
            args = [c2, *args]
        elif m is QualityHead:
            chs = [ch[x] for x in f]
            args = [tuple(chs), *args]
            c2 = chs[0]
        ...
        def _flatten_from(index):
            if isinstance(index, int):
                return [] if index == -1 else [index]
            flat = []
            for x in index:
                if isinstance(x, list):
                    flat.extend(_flatten_from(x))
                elif x != -1:
                    flat.append(x)
            return flat

        save.extend(x % i for x in _flatten_from(f))  # append to savelist
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if "detect" in m:
            return "detect"
        if "cargodefect" in m or "cargoinspection" in m or "smalldefect" in m:
            return "detect"
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, SemanticSegment):
                ...
                return "detect"
            elif isinstance(m, (CargoDefectHead, CargoInspectionHead)):
                return "detect"
