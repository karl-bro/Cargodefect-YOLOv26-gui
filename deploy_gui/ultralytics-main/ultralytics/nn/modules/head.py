...
class SmallDefectHead(Detect):
    """Lightweight P2/P3 small defect detection head (DWConv + 1x1 Conv)."""

    def __init__(self, nc: int = 6, reg_max=1, end2end=False, ch: tuple = ()):
        super().__init__(nc, reg_max, end2end, ch)
        self.cv2 = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), Conv(x, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = nn.ModuleList(nn.Sequential(DWConv(x, x, 3), Conv(x, self.nc, 1)) for x in ch)
        if end2end:
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)
...
class CargoDefectHead(nn.Module):
    """Composite head for CargoDefect-YOLOv26: cargo detection + small defect + quality OK/NG."""
    ...
    def __init__(
        ...
        ch_cargo: tuple,
        ch_defect: tuple,
        use_small_defect_head: bool = True,
        ...
        self.use_small_defect_head = use_small_defect_head
        self.use_quality_head = use_quality_head

        self.detect = Detect(nc, reg_max, end2end, ch_cargo)
        self.small_defect = (
            SmallDefectHead(nc_defect, reg_max, end2end, ch_defect) if use_small_defect_head and ch_defect else None
        )
        self.quality = QualityHead(ch_cargo, nc_quality, quality_thresholds) if use_quality_head else None
    ...
    def end2end(self, value):
        ...
        if self.small_defect is not None:
            self.small_defect.end2end = value
    ...
    def stride(self, value):
        ...
        if self.small_defect is not None:
            self.small_defect.stride = value
    ...
    def inplace(self, value):
        ...
        if self.small_defect is not None:
            self.small_defect.inplace = value

    def bias_init(self):
        """Initialize detection head biases."""
        self.detect.bias_init()
        if self.small_defect is not None:
            self.small_defect.bias_init()

    def forward(self, x: list) -> dict | torch.Tensor | tuple:
        """Forward cargo detect, optional defect detect, and quality classification."""
        if isinstance(x[0], list):
            cargo_feats, defect_feats = x[0], x[1] if len(x) > 1 else x[0][:2]
        else:
            cargo_feats, defect_feats = x, x[:2] if len(x) >= 2 else x

        detect_out = self.detect(cargo_feats)
        outputs = {"detect": detect_out}

        if self.small_defect is not None:
            outputs["defect"] = self.small_defect(defect_feats)

        if self.quality is not None:
            det_preds = detect_out if isinstance(detect_out, dict) else detect_out[1]
            if isinstance(det_preds, dict) and "one2many" in det_preds:
                det_preds = det_preds["one2many"]
            outputs["quality"] = self.quality(cargo_feats, det_preds)
