#!/bin/bash
# Train CargoDefect-YOLOv26-Detect: PGME backbone + 4-scale neck + standard Detect Head
# Training from scratch (YAML build) since PGME + 4-scale architecture is unique
set -e
cd "$(dirname "$0")/.."
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate cargodefect

MODEL_YAML="ultralytics/cfg/models/26/cargodefect-yolov26-detect.yaml"
DATA_YAML="ultralytics/cfg/datasets/cargodefect-package.yaml"
NAME="cargodefect_yolov26_detect"
