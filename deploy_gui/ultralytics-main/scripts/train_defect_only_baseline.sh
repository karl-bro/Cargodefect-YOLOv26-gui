#!/bin/bash
# Train YOLOv26 baseline — DEFECT-ONLY (nc=1)
# Standard Detect head, no CargoInspectionHead
set -e
cd "$(dirname "$0")/.."
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate cargodefect

MODEL_YAML="ultralytics/cfg/models/26/yolov26n-defect-only.yaml"
DATA_YAML="ultralytics/cfg/datasets/cargodefect-defect-only.yaml"
NAME="defect_only_baseline"
...
print('defect_only_baseline DONE')
"
